...
public function index(Request $request)
{
    ...
    // When there's no search term, we skip the GROUP_CONCAT entirely
    // to avoid an unnecessary GROUP BY on every page load.
    if ($search) {
        $ldSubquery = DB::raw('(
            SELECT employee_id,
            ...
            WHERE title_of_learning_and_development_interventions LIKE ' . DB::connection()->getPdo()->quote("%{$search}%") . '
            GROUP BY employee_id
        ) AS ld_agg');
    } else {
        $ldSubquery = DB::raw('(
            SELECT NULL AS employee_id, NULL AS matched_training
            LIMIT 0
        ) AS ld_agg');
    }
    ...
    // Paginate
    $employees = $query
        ->orderBy('personal_information.surname', 'asc')
        ->paginate(10)
        ->withQueryString();

    // AJAX: return only the table partial for smooth search
    if ($request->ajax()) {
